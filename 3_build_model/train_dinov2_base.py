...
# 用于命名输出文件的别名
MODEL_ALIAS = "DINOv2_base"

# 数据和训练参数
DATA_DIR = 'data'
BATCH_SIZE = 8  #  只能用 8, 此时显存是 9 / 12G
EPOCHS = 20     # 训练20轮，以获得稳定的结果
LEARNING_RATE = 1e-4
...
# 核心：使用timm.create_model并设置num_classes=2，timm会自动处理好头的替换，最安全的方式！
...
if class_counts.size > 1 and class_counts[0] > 0 and class_counts[1] > 0:
    class_weights = torch.tensor([max(class_counts ) /class_counts[0], max(class_counts ) /class_counts[1]], dtype=torch.float32).to(device)
