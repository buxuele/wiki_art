...
# --- 1. 配置 ---
# --- 核心修改：更换模型路径和别名 ---
MODEL_PATH = "timm/vit_base_patch16_224.dino"
MODEL_ALIAS = "DINO_base_timm"  # 给它一个新名字以区分

# 数据和训练参数
DATA_DIR = 'data'
# DINO Base模型比DINOv2 Base更轻量，我们可以尝试稍大的Batch Size
BATCH_SIZE = 32        # 32 此时的显存占用 5.8G
EPOCHS = 30            # 给予足够多的训练时间
LEARNING_RATE = 1e-4   # 使用一个经过验证的、适合微调的通用学习率
...
# 使用timm.create_model并设置num_classes=2，timm会自动处理好头的替换
...
if class_counts.size > 1 and class_counts[0] > 0 and class_counts[1] > 0:
    # 权重计算公式调整，给少数类（good）更高的惩罚
    weight_for_good = class_counts[0] / class_counts[1]
    class_weights = torch.tensor([1.0, weight_for_good], dtype=torch.float32).to(device)
